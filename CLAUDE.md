...
- **`pdf-lib` is confined to `render.ts` — its only import site (grep-verified, keep it that way).** It exists solely to MERGE the per-sheet-group PDFs Phase 7 renders (one `renderPdf` per traveler load / per shipping-ticket order, so each sheet group gets its OWN "Page N of M" and continuation header — pdfmake's callbacks only see whole-document page counts, and pdfmake cannot merge). Definitions stay pure JSON; the footer/continuation-header specs are declarative data and the pdfmake/pdf-lib callbacks live only inside `render.ts`. Do not reach for `pdf-lib` from a builder or a service.
