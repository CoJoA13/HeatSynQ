...
**The GL-export delta (Phase 5C).** Postings live in an append-only `GlPosting` ledger keyed on `(sourceType, sourceId)`; the delta is computed **strictly per-period** (`[monthStart, monthEnd]`, never cumulative — the period lock keeps a closed month stable, and cumulative bounds would double-post when months export out of order). Readiness must refuse an export for **every** account-bearing non-`TAX` invoice line lacking a GL account (freight/charge/cert too, not only step-code/surcharge), and `exportClose` asserts Σdebit = Σcredit before persisting. Nothing is cached on `Invoice`/`Payment`/`Application`; balances derive live.
