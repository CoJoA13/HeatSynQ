...
> `docs/company-confidential/2026-08-17-accounting-questions.md`). **33 issues open. NEXT when
> asked: Group D — and it OPENS with the owner's #31 decision** (the stale-load class: #3, #5,
> #15, #23, #31, #110; the ruling shapes the sweep, which is why D was deliberately not early —
> Group C's `travelerPrinted` monotonic merge is a live exhibit of the class, cited in its
> ledger). After D: **F** infra → **G**/**H** as filler.
>
