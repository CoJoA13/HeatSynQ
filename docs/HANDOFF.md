...
**`manual:capture && manual:build` produces a publishable page with no hand step, and the figure-sizing rule no longer hardcodes the capture scale (#169, gate-infrastructure Task 5, 2026-08-22).** `manual.html` fitted under the 16 MB publish ceiling only because `magick -colors 256 -depth 8 -strip` had been run over `docs/manual/img/` by hand, twice, in two sessions, in a step recorded nowhere — so the documented workflow on a fresh checkout produced a ~28 MB page that cannot be published, with nothing saying why. **Capture is now 1:1** (`DEVICE_SCALE`, `MANUAL_SCALE ?? 1`): the manual lays a full-width figure out at 1200 declared px and renders it at **800** (`.content` is `max-width:50rem`; measured in a browser, not assumed), so 2x density was never reaching a reader — only the byte count. **The coupled half is what makes it safe:** `build-manual.mjs` computed display size as `round(intrinsic × 10/24)`, and that constant *is* `1200/2880` — the DSF-2-at-1440 assumption, hardcoded. Demonstrated rather than argued: with the old factor and the new capture, `login.png` declares **600×375**, half size, every figure. The rule is resolution-independent now — the column width when the image is at least that wide, else its intrinsic width, height by the same ratio — living in `erp/scripts/lib/manual-figure-size.mjs`, a pure leaf split out only so a test can reach it (`build-manual.mjs` runs `build()` at module scope; the `e2e/lib/failure-classify.mjs` precedent). That **also fixes a live distortion the issue predicted**: an image narrower than the column was being shrunk by the same 10/24 — a 600px clip declared at 250 — proven end-to-end by building with a 600×360 probe figure (`600×360` declared, `600` rendered) and reverted. No current figure was affected, because every capture today is a full-page shot at viewport width; the defect was latent, not live. **Measured: 13,062,687 bytes** of the 16,777,216 ceiling (77.9%, 3.54 MB of headroom) from `img/` at 10.0 MB, against 15,317,815 bytes (91.3%) hand-compressed — **no ImageMagick, no image encoder, no system dependency**, which was the acceptance test. `manual:build` stays zero-dependency and deterministic: two builds byte-identical, and identical again across an intervening mutate-and-revert cycle (sha256 `f9d8b1e9…`). The capture's sweep is clean — 50 PASS, 0 WARN/FAIL/ERROR/SKIPPED. **`invoicing-detail.png` is the one figure #170 is still costing:** 2974×2868 because the invoice page overflows horizontally, 1,213,061 bytes on disk / **1.54 MB inlined, 12.4% of the whole page** — at the other 49 figures' measured 0.0877 bytes/px, the same screen at 1440 wide would cost roughly 0.36–0.75 MB depending on how much taller the reflowed JSON makes it, so fixing #170 frees roughly **0.6–1.1 MB**. Its raw JSON is unreadable in the manual anyway: 2974 intrinsic px shown at 800. **The lint gate is `npx eslint src tests e2e scripts prisma`** — Task 3 found `scripts/` and `prisma/` outside it, so `build-manual.mjs` had never been linted once; both are clean today and both are cheap (`prisma/generated/**` is already in `ignores`), so both were added rather than only the one being edited.
