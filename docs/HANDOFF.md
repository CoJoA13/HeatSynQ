...
**Phase 2B is finished and open as a pull request, not merged.** `phase-2b-customers`, pushed, `MERGEABLE`, 254 tests with `tsc` / `eslint` / `npm run build` all clean. **PR: https://github.com/CoJoA13/HeatSynQ/pull/2**

Seven rounds of automated review have run against it. **All 37 threads are answered and resolved.** Thirty-one were fixed on the branch; five were filed as issues; one was answered as already-recorded. Open issues — all deliberate deferrals or owner decisions, none an oversight. **#6, #7 and #8 are already decided; their rulings are §5.14–§5.17 and 2C builds them:**
