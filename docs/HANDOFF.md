...
**Phase 2B is finished and open as a pull request, not merged.** `phase-2b-customers` at `047eb51`, pushed, `MERGEABLE`, 251 tests with `tsc` / `eslint` / `npm run build` all clean. **PR: https://github.com/CoJoA13/HeatSynQ/pull/2**

Four rounds of automated review have run against it. **All 28 threads are answered and resolved.** Twenty-four were fixed on the branch; three were filed as issues; one was answered as already-recorded. Open issues, all deliberate deferrals rather than oversights:
...
- **#6** — **decided by the owner 2026-07-31; 2C builds it.** See §5.14 for the rule and the reasoning.
...
1. Check the PR for a fifth review round (`gh pr view 2`, and the GraphQL `reviewThreads` query for unresolved ones). Triage by §5a's rule, reply on each thread, resolve.
...
3. Phase 2C is Parts. Its plan is not written. Its inputs: the kickoff brief (`docs/superpowers/plans/2026-07-30-phase-2-kickoff.md`, §2.3 and open items), the process-steps model doc, and §6's carried backlog. **Four obligations it inherits:** consolidate revival-on-create into one shared helper before parts add a fifth site; build name resolution for the raw-cuid reference columns; build the reference-delete guard ruled in §5.14; and decide the reference-pick-list permission question (§6), which parts need four times over on one screen — the one item on this list still owed an owner decision.
