...
| 9 | Restore runbook + E2E flow + docs | sonnet, DONE | (no per-task review dispatched — this task closes with the whole-branch review instead, per the controller's direct instruction) | New `e2e/flows/backups.mjs` (23rd/last FLOWS entry) + the 4 harness plumbing edits (`BACKUP_DIR` beside `ARTIFACTS_DIR`, spawn env, `main()` reset, shared `teardown()`); the expanded restore runbook + Backups section in `erp/README.md` (verified against the real compose setup — `db` has no profile so `docker compose exec -T db pg_dump …` needs no `--profile` flag while `app` does; added a `pg_terminate_backend` step before `DROP DATABASE` that the brief's draft lacked, since a stray psql/Prisma-Studio session on the published 5432 port would otherwise make the drop refuse); the CLAUDE.md architecture paragraph (verbatim from the brief, cross-checked against `backup-paths.ts`/`backups.ts`/`scripts/backup.sh` for accuracy before landing); HANDOFF §4/§8/§9 (8C marked code-complete pending whole-branch review + merge, `postgresql` added to the Fedora tooling list, §9 rewritten to the three post-8C tracks). **Found and fixed one pre-existing bug outside the task's own file list**: `tests/backup-paths.test.ts`'s "defaults to /backups when BACKUP_DIR is unset" test called `resolveBackupDir(undefined)`, but that only exercises the hardcoded fallback if `process.env.BACKUP_DIR` is ALSO unset — and Task 8's `.env.example` addition (`BACKUP_DIR="./backups"`) leaks into `process.env` via `tests/helpers/setup.ts`'s unconditional `dotenv.config()`, so the full suite reproducibly failed 1/2984 on this branch (confirmed pre-existing via `git stash`). Fixed with the same save/restore idiom `tests/backups-routes.test.ts` already uses for the same env var. **Two judgment calls (both resolved in favour of the flow as shipped, one with a change):** (1) the restore runbook's ordering was verified command-by-command against the live compose setup (harmless `pg_dump --schema-only`, a `SELECT 1` against the `postgres` maintenance db) rather than trusted as written; (2) the brief flagged the third E2E assertion (staleness bar clears after `page.goto("/customers")`) as a possible throttle/latch race — analysis showed `page.goto` is a full browser navigation that tears down `BackupBanner`'s `useRef`-held throttle state entirely, so the 5-minute throttle never actually spans the assertion; the REAL race was a fetch-timing one (asserting immediately after `goto` could vacuously pass before the health fetch even resolves), fixed by arming `page.waitForResponse` on `/api/admin/backups/health` before navigating and awaiting it before the assertion. Final gates (all watched to completion): **2984 tests / 179 files**, `tsc`/`eslint`/`build` clean, **E2E 23/23** (the full run, headless, ~9 min). |
