...
- [ ] **Step 2: Update `src/app/parts/[id]/page.tsx`** — delete `setupCharge` / `unitPrice` / `minimumCharge` / `pricePer` from the `Part` type and from anything that spreads it into `save()`. `PricingSection` no longer needs `save` or `patchDraft` props at all; it owns its own fetches. Remove the `// TASK 5:` marker Task 2 left.
...
- [ ] **Step 1: Widen `documents.ts`** — `AREA_FOR_KIND` gains `INVOICE: "invoicing"` and `CREDIT: "invoicing"`; `DocumentOwner` gains `| { kind: "INVOICE"; invoiceId: string } | { kind: "CREDIT"; invoiceId: string }`; `ownerColumns` gains both arms (every other owner column null); `listDocumentsForOrder`'s `OR` gains `{ invoice: { orderId } }`; `documentFilename` yields `invoice-72026.pdf` / `credit-1000.pdf`; `resolveDocumentFilename` gains both cases. `Record<DocumentKind, Area>` makes a missing entry a **compile error**, which is the point.
