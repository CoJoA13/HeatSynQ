...
| §5.10 early-pay discount basis — **the AMOUNT BEING PAID** (issue #69) | The discount is a percentage of what the customer actually remits early, not of the open balance (as built) nor of the original total: \$1,000 at 2/10 Net 30 paid \$500 inside the window earns \$10. It can never give away more than was earned on money actually received early. **Two read sites must move together** — `discountAvailable` offers the figure and `applyPayment` caps the DISCOUNT line independently; the frozen `termsDiscountPercent`/`termsDiscountDays` (#79) stay the percentage source, only the base changes |
| §9 "destructive-ish" **defined**: cascades or frees an identifier (issue #8) | A delete requires a reason when it takes other records with it or frees a unique code for reuse — customer (built) and **role** (clears permission grants). Addresses, contacts, reference rows and step codes stay promptless: a reason prompt in front of routine cleanup is the cost this reading deliberately avoids. If the §5.14 guard is ever relaxed so a reference delete can proceed while records point at it, that delete joins the list |
