...
| §5.14 the hidden discount offer **must name its route out** (issue #155 arm 2) | With terms of 2/10 and a payment inside the window, an operator entering a partial payment sees **nothing at all** in the Discount column — no control, no explanation — and cannot learn that remitting 980.00 instead of 500.00 would earn 20.00. The offer read returns **why** the figure is zero, and the operator-fixable case renders a hint naming the settling figure; `no terms discount` and `window closed` stay silent, because there is genuinely no route out of either. **Text-only, not a disabled control** — two E2E flows assert a row-scoped checkbox count of 0 on their partial applies, which a hint keeps green and a disabled checkbox would correctly fail. Arm 1's ruling is what makes the hint's wording stable: the figure that earns the discount is always the settling one |
