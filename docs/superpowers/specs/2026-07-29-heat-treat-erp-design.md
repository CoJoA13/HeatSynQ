...
| §9 `apply_payments` is **enforced**, not just declared (issue #211, ruled 2026-08-25) | The named dangerous action existed from Phase 5B as a roles-editor checkbox, a per-user override target, and a spec §9 list entry — and no route ever checked it, so denying it did nothing while the manual taught admins to rely on it. **Ruled: wire it** (the alternative was removal). `POST /api/receivables/applications` and `POST /api/receivables/credit-applications` now take `mustDo(apply_payments)` on top of `receivables.create` — recording payments and batches stays create-only; *moving cash or credit onto invoices* is the guarded act. The standalone write-off route is deliberately NOT widened (create + `write_off` unchanged — a write-off applies no payment), and the customer page's write-off gate composes off the plain create gate for exactly that reason (the UI gate must match the route it calls). A backfill migration (`20260825213000_…`) grants the action to every live role holding `receivables.create` — preserving what each role could already do, conferring nothing new (the `manage_backups` backfill's intent, with its predicate-that-does-not-decay lesson applied from the start) |
| §5.13 a destructive control **REFUSES over unsaved edits unless the destruction is what was asked for**, and the refusal is scoped to the rows it destroys (issue #297, ruled 2026-09-07) | CLAUDE.md's rule said a control that archives paper or replaces server rows must refuse while an editor is dirty — *never merely warn* — and the same paragraph listed three in-page destroyers that legitimately ask first, with load re-split appearing on both sides of the line. A mechanical sweep (#277) cannot be written against a rule that contradicts itself, so it recorded the tension instead. **Ruled: the line is not refuse-vs-ask, it is whether the destroyed rows are the ones the operator NAMED.** Re-split recomputes the loads because they clicked re-split; shipment reverse replaces the shipment because they clicked reverse — the prompt confirms the action, and asking is right. `removeLine` (#293) and `removeOrderFromShipper` (#294) destroy a *different* section's rows as a side effect, so they refuse. **The refusal had to become scoped for that to be usable**: every line's serials editor registers the same human label, so the registry could answer only "is anything dirty" and a page-wide refusal would have blocked removing a line because an unrelated Charges grid was dirty. The registry now carries a `scope` beside the label (`serials:<lineId>`, `shipment-order:<shipperOrderId>`); scoping by relabelling instead is rejected outright, because the traveler print gate matches labels by exact equality and per-line labels would silently stop it recognising the section it exists to ignore. Both ends of a scoped gate are mechanically proved, the ask is proved per control rather than per file, and the pinned count of known-unguarded controls is **0** |
