...
- A **discount** appears only when the invoice's terms carry one **and** the payment's `receivedDate ≤ invoiceDate + discountDays`; the system computes the eligible amount (`discountPercent / 100 × the amount being settled`), the operator chooses to take it. A `DISCOUNT` application carries the reason "early-pay terms".
