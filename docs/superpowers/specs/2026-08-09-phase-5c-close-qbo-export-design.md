...
| 8 | Invoice recognition basis (owner ruling 2026-08-10, from the whole-branch review) | **An invoice counts in the month it is FINALIZED/locked (`finalizedAt`), not its document date (`invoiceDate`).** An invoice dated 2026-07-31 but finalized 2026-08-02 is August activity. This is the A/R-correct basis — you don't owe on an unlocked invoice, so 5B's aging already includes an invoice by `finalizedAt` — and it makes the close's roll-forward reconcile to the aging. **It applies consistently to all three period-scoped mechanisms:** the close roll-forward (`computeSchedule` scopes finalized invoices/credits by `finalizedAt`), the GL export (`gl-export.ts` scopes finalized invoices/credits by `finalizedAt`), and the **period lock** — `finalize` guards on the finalize date (≈ today), `unlock`/void-order guards on the invoice's own `finalizedAt` (the month whose figures the unlock would change). Keeping the lock on `invoiceDate` under a finalize-basis scope would let unlocking a July-dated / August-finalized invoice change August's frozen figures while the lock only checked July — the same closed-month leak fixed in Task 5. `invoiceDate` remains the document date printed on the invoice; payments/applications keep their `receivedDate`/`appliedDate` cash-basis dates (unchanged). |
| 9 | Export file granularity (owner ruling 2026-08-10) | **Summary journal — one line per GL account per side**, detail left in the ERP (spec §1/§7.6). The `GlPosting` rows stay the per-event ERP-side detail and the delta driver; `renderCsv` and the posting register **aggregate** the emitted lines by `(glAccountName, side)` into one journal line per account before writing. (An earlier plan draft said "one row per line" — a plan error, corrected here.) |
