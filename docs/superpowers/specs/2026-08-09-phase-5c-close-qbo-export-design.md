...
**The delta contract — export is per-event, bounded by the exported period's end.** Exporting close period *P* (period-end **E**) works entirely within `glDate ≤ E`, so re-exporting an earlier month after a later one has closed never disturbs the later month (its events are out of scope). Per event, keyed by `(sourceType, sourceId)`:

- **In-scope live events** = the current postable events (finalized non-discarded invoices/credits; posted non-void payments; live discount/write-off applications) with GL-date ≤ E.
- **Net prior postings** = the `GlPosting` rows with `glDate ≤ E`, grouped by `(sourceType, sourceId)`, with reversals cancelling originals.
...
Spec §15's "refuse to export rather than post without an account" is the **§5.14 name-the-blockers** pattern applied to the GL. Before an export, `gl-mapping.ts` resolves every postable event to its account and returns the **unresolved set**: step codes with `needsGlAccount`/no `glAccountId` that appear on a postable invoice line, surcharges with no GL, payment types with no GL used by a postable payment, and any missing plant default (`arGlAccountId` absent blocks everything; `discountGlAccountId`/`writeOffGlAccountId` absent blocks only if a discount/write-off is in the delta). Each entry links to the record that fixes it and the whole list is **Excel-exportable**. The export button is disabled with the count until the list is empty; the close is not blocked by it (you can close a month and key the accounts before exporting).
...
- **Sweeps:** `gl_export_next` joins the allocation-number guard set; `GlExportBatch.exportNumber` carries the documented allocation-only `@unique` exemption beside `creditNumber`/`batchNumber`. The three new `BillingConfig → GlAccount` FKs join the **§5.14 delete-blocker registry**, so deleting a GL account wired as a plant default is refused and names the blocker. `ClosePeriod`/`GlExportBatch`/`GlPosting` are not soft-deletable → no partial-unique columns, no findUnique/upsert exposure.
...
- **Concurrency** — `assertPeriodOpen` verified **RED with the guard removed** and the competing caller pinned to Read Committed (CLAUDE.md): a finalize/apply/void racing a close of the same month; the close claiming under a lock so two closes of one month can't both write. Periods-close-in-order and single-close-per-month proven under contention.
- **Sweeps** — partial-unique, permissions (the two new specials), the delete-blocker registry (the three new FKs), and the allocation-number guard (`gl_export_next`) stay green.
