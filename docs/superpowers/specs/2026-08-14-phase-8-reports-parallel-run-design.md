...
| **A manual backup is audited** | A dump is a full copy of every customer's data and `manage_backups` is on the dangerous-action list, so "who dumped production, and when" is an audit question. It has no entity row, so it uses the **`auditSettingChange` sanctioned-exception shape** with a sweep-allowlist entry |
...
| **E2E coverage of "Back up now"** | The owner installs the **`postgresql` client on the dev host** (`sudo dnf install -y postgresql`) so the Playwright flow clicks the real button and asserts a genuine archive — the headline control is proven end to end, not only in vitest. This adds a host prerequisite to HANDOFF §8's fresh-machine setup |
