...
// Mirrors src/app/api/admin/reference/[kind]/[id]/blockers/export/route.ts exactly — same
// gating, same toXlsx column shape — for the one BlockerTarget that isn't a ReferenceKind
// (spec §7).
export const GET = handle(async (_req, { params }) => {
  mustCan(requireUser(), "admin", "view");
  const { id } = await params;
  const blockers = await findBlockers("processStepCode", id);
