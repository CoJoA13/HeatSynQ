...
export const GET = handle(async () => {
  ...
  // projection is the narrowest possible (id/name/docType of live templates + a derived `published`
  // boolean — no configs, no counts; Task 20 pre-step: `published` lets the picker disable a
  // never-published template with its §5.16 tooltip instead of surfacing the assign-time 400);
  // managing templates stays behind the `templates` area. Bound to a variable (not a
